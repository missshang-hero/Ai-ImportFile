...
export default function useTable(params: any) {
  ...
  const isCommonQuery = customConfig.isCommonQuery || false // 是否通用查询
  const pageSize = customConfig.pageSize || 15 // 自定义每页搜索数量value 通用查询默认100 非通用查询默认15
  ...
  const [pagination, setPag] = useState({
    ...
    onChange: (current = 1, pageSize = isCommonQuery ? 100 : 15, config?: any) => {
